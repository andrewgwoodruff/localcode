...
const disableDefault = process.env.LOCALCODE_DISABLE_DEFAULT_PLUGINS
process.env.LOCALCODE_DISABLE_DEFAULT_PLUGINS = "1"

const { Plugin } = await import("../../src/plugin/index")
const { PluginLoader } = await import("../../src/plugin/loader")
const { readPackageThemes } = await import("../../src/plugin/shared")
const { Instance } = await import("../../src/project/instance")
const { Npm } = await import("@localcode/core/npm")

afterAll(() => {
  if (disableDefault === undefined) {
    delete process.env.LOCALCODE_DISABLE_DEFAULT_PLUGINS
    return
  }
  process.env.LOCALCODE_DISABLE_DEFAULT_PLUGINS = disableDefault
})
...
`,
  ...
  test("skips external plugins in pure mode", async () => {
    ...
    const pure = process.env.LOCALCODE_PURE
    process.env.LOCALCODE_PURE = "1"
    ...
    try {
      ...
      if (pure === undefined) {
        delete process.env.LOCALCODE_PURE
      } else {
        process.env.LOCALCODE_PURE = pure
      }
    }
  })
