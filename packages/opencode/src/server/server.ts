...
export const Default = () => (Flag.LOCALCODE_EXPERIMENTAL_HTTPAPI ? DefaultHttpApi() : DefaultHono())

function create(opts: { cors?: string[] }) {
  if (Flag.LOCALCODE_EXPERIMENTAL_HTTPAPI) return createHttpApi()
...
function createHono(opts: { cors?: string[] }) {
  ...
  if (Flag.LOCALCODE_WORKSPACE_ID) {
    return {
      app: app
        .use(InstanceMiddleware(Flag.LOCALCODE_WORKSPACE_ID ? WorkspaceID.make(Flag.LOCALCODE_WORKSPACE_ID) : undefined))
        .use(FenceMiddleware)
