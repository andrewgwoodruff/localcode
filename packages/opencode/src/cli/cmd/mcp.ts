...
import { InstallationVersion } from "@localcode/core/installation/version"
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files. Order matches the runtime loader's
  // precedence (config.ts:337): localcode.{json,jsonc} preferred, then
  // opencode.{json,jsonc} as legacy fallback.
  const candidates = [
    path.join(baseDir, "localcode.json"),
    path.join(baseDir, "localcode.jsonc"),
  ...
  if (!global) {
    candidates.push(
      path.join(baseDir, ".localcode", "localcode.json"),
      path.join(baseDir, ".localcode", "localcode.jsonc"),
