...
import { InstallationLocal, InstallationVersion } from "@localcode/core/installation/version"
import { makeRuntime } from "@localcode/core/effect/runtime"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // folders between cwd and home, and LOCALCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.LOCALCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  if (Flag.LOCALCODE_TUI_CONFIG) {
    const configFile = Flag.LOCALCODE_TUI_CONFIG
  ...
  // 4. `.opencode` directories (and LOCALCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter(
    (dir) => dir.endsWith(".localcode") || dir.endsWith(".opencode") || dir === Flag.LOCALCODE_CONFIG_DIR,
  )

  for (const dir of dirs) {
    if (!dir.endsWith(".localcode") && !dir.endsWith(".opencode") && dir !== Flag.LOCALCODE_CONFIG_DIR) continue
