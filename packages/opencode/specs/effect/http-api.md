...
- `LOCALCODE_EXPERIMENTAL_HTTPAPI` gates the bridge. Default behavior still uses Hono.
...
3. Mount the new paths in `src/server/routes/instance/index.ts` only inside the `LOCALCODE_EXPERIMENTAL_HTTPAPI` block.
...
2. The route is mounted by default, not only behind `LOCALCODE_EXPERIMENTAL_HTTPAPI`.
...
- [x] Bridge selected `HttpApi` routes into Hono behind `LOCALCODE_EXPERIMENTAL_HTTPAPI`.
