...
:::note
This tool is only available when using the OpenCode provider or when the `LOCALCODE_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).
