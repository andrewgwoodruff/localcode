...
Avvia un server HTTP che espone accesso API alle funzionalità di opencode senza la TUI. Imposta `LOCALCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Avvia un server HTTP e apre un browser per accedere a OpenCode tramite interfaccia web. Imposta `LOCALCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
| `LOCALCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `opencode`)     |
...
| `LOCALCODE_EXPERIMENTAL_LSP_TY`                  | boolean | Abilita TY LSP per i file python           |
